...
- **Errors:** throws `EnvironmentValidationError` (never a raw Zod error) listing every
  invalid/missing field, and never includes any variable's _value_ in the message —
  ...
  `SUPABASE_MANAGEMENT_OAUTH_REDIRECT_URI` / `SUPABASE_URL` not a valid URL;
  `SUPABASE_PUBLISHABLE_KEY` or `SUPABASE_SECRET_KEY` missing; `NODE_ENV` outside
  `development`/`test`/`production`; `PORT` outside 1–65535; neither
