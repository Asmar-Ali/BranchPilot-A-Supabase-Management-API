...
- **Errors:** missing, expired, or replayed state returns `400 OAUTH_STATE_INVALID`; token
...
- **Errors:** `invalid_grant` marks a connection revoked and yields `409 SUPABASE_REAUTH_REQUIRED`.
  A transient revocation failure leaves `revocation_pending` and returns retryable `503`.
